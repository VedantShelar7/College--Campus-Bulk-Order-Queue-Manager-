  ...
  if (completed) {
    ...
    if (!done.length) {
      ...
      completed.innerHTML = done.map(o => {
        ...
        const tokenBadge = o.tokenId ? `<span class="badge badge-primary" style="font-weight:700; letter-spacing:0.5px; margin-right:6px; font-size:0.7rem;">${o.tokenId}</span>` : '';
        const crName = o.assignedCR ? ` · CR: ${o.assignedCR.name}` : '';
        return `<div class="flex items-center justify-between" style="padding:10px 0;border-bottom:1px solid var(--border-color);">
          <div><div class="font-semibold text-sm">${tokenBadge}${o.student}</div><div class="text-xs text-muted">${o.poll} · ${o.pages} pages${crName}</div></div>
...
function renderOrdersTable() {
  ...
  if (!orders.length) {
    tbody.innerHTML = '<tr><td colspan="10" class="text-muted text-center">No orders found.</td></tr>';
    return;
  }
  tbody.innerHTML = orders.map((o, i) => {
    ...
    // Token display
    const tokenDisplay = o.tokenId
      ? `<span class="badge badge-primary" style="font-weight:700; letter-spacing:0.5px;">${o.tokenId}</span>`
      : '<span class="text-muted">—</span>';

    // CR display
    const crDisplay = o.assignedCR
      ? `<span class="font-semibold" style="color:var(--primary);">${o.assignedCR.name}</span>`
      : '<span class="text-muted">—</span>';
