...
function renderCRPendingPickups() {
  ...
  container.innerHTML = items.map(o => {
    const tokenBadge = o.tokenId ? `<span class="badge badge-primary" style="font-weight:700; letter-spacing:0.5px; margin-right:6px;">${o.tokenId}</span>` : '';
    const crLabel = o.assignedCR ? `<span class="text-xs text-muted">CR: ${o.assignedCR.name}</span>` : '';
    if (o.paymentStatus === 'submitted') {
      ...
      <div style="padding:12px; border:1px solid #FCD34D; border-radius:8px; margin-bottom:12px; background:#FEF3C7; display:flex; justify-content:space-between; align-items:center;">
        <div>
          <div class="font-semibold text-sm" style="color:#92400E;">${tokenBadge}${o.student}</div>
      ...
      <div style="padding:12px; border:1px solid var(--border-color); border-radius:8px; margin-bottom:12px; background:var(--bg-secondary); display:flex; justify-content:space-between; align-items:center;">
        <div>
          <div class="font-semibold text-sm">${tokenBadge}${o.student}</div>
...
async function createPrintPoll() {
  ...
  const tokenInput = document.getElementById('poll-token-id');
  const crSelect = document.getElementById('poll-assign-cr');

  // Validation
  if (!cid || !title?.value.trim() || !price?.value) { showToast('warning', 'Missing Fields', 'Select classroom, fill title & price.'); return; }
  if (!tokenInput?.value) { showToast('warning', 'Token Missing', 'Token ID was not generated. Please close and reopen the modal.'); return; }
  if (!crSelect?.value) { showToast('warning', 'CR Required', 'Please select an Assigned CR before submitting.'); return; }

  // Build assignedCR object from select
  ...
  try {
    const res = await fetch(API + '/polls', {
      ...
      body: JSON.stringify({
        ...
        duration: parseInt(dur?.value || 60),
        document: docData,
        qrCode: _qrDataUrl,
        tokenId,
        assignedCR
    ...
    const data = await res.json();
    if (res.ok) {
      showToast('success', 'Poll Created!', `"${title.value.trim()}" is now live. Token: ${tokenId}`);
...
function renderStatusQueue() {
  ...
  container.innerHTML = orders.map(o => {
    ...
    const tokenBadge = o.tokenId ? `<span class="badge badge-primary" style="font-weight:700; letter-spacing:0.5px; margin-right:6px; font-size:0.7rem;">${o.tokenId}</span>` : '';
    const crName = o.assignedCR ? `<span class="text-xs" style="color:var(--primary); margin-left:4px;">👤 ${o.assignedCR.name}</span>` : '';
    return `<div class="queue-item">
      <div class="queue-item-number">${pos > 0 ? '#' + pos : '—'}</div>
      <div class="queue-item-info"><div class="font-semibold text-sm">${tokenBadge}${o.student}</div><div class="text-xs text-muted">${o.poll} · ${o.file} · ${o.pages} pages${crName}</div></div>
    ...
    if (act === 'open-create-poll') {
      ...
      if (modal) {
        ...
        do {
          ...
          tokenStr = `PR-${yr}-${rand}`;
        } while (_polls.some(p => p.tokenId === tokenStr) || _orders.some(o => o.tokenId === tokenStr));
        const tokenInput = document.getElementById('poll-token-id');
        if (tokenInput) tokenInput.value = tokenStr;

        // 2. Fetch CRs
        fetch(API + '/coordinators', { headers: authHeaders() })
          .then(r => r.json())
          .then(data => {
            const select = document.getElementById('poll-assign-cr');
            if (select) {
              const user = getLoggedInUser();
              select.innerHTML = '<option value="" disabled>-- Select CR --</option>' +
                data.map(cr => `<option value="${cr.id}" data-name="${cr.name}" data-email="${cr.email}" ${cr.email === user.email ? 'selected' : ''}>${cr.name}</option>`).join('');
            }
          }).catch(console.error);
