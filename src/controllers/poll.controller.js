...
function createPoll(req, res) {
  const { classroomId, title, desc, price, duration, document: doc, qrCode, tokenId, assignedCR } = req.body;
  if (!classroomId || !title || !price || !tokenId || !assignedCR) {
    return res.status(400).json({ error: 'Classroom, title, price, Token ID, and Assigned CR are required' });
  }
  ...
  // Basic validation to ensure token isn't already used
  if (polls.some(p => p.tokenId === tokenId)) {
    return res.status(400).json({ error: 'Token ID already exists. Please regenerate.' });
  }
  ...
  const poll = {
    ...
    assignedCR: {
      id: assignedCR.id,
      name: assignedCR.name,
      email: assignedCR.email
    },
    classroomId,
